...
func (accountRouter *AccountRouter) RegisterRoutes(router *gin.RouterGroup, jwtService *services.JwtService) {
	accountRou := router.Group("/accounts")
	{
		...
		accountRou.POST("", middlewares.AuthorizeJWT(jwtService), accountRouter.accountController.CreateAccount)
		...
		accountRou.PATCH("/soft-delete", middlewares.AuthorizeJWT(jwtService), accountRouter.accountController.SoftDelete)
		accountRou.GET("", middlewares.AuthorizeJWT(jwtService), accountRouter.accountController.SearchAccount)
